...
class LLMMCPHardwareServer:
    ...
    async def get_hardware_client(self):
        ...
        if self._hardware_client is None or not getattr(self._hardware_client, "is_connected", False):
    ...
    def _register_default_tools(self):
        ...
        @self.mcp.tool()
        async def record_audio(duration: int = 3, sample_rate: int = 44100, channels: int = 1) -> Dict[str, Any]:
            """
        ...
        @self.mcp.tool()
        async def get_sensor_reading(sensor_type: str, pin: Optional[int] = None) -> Dict[str, Any]:
            """
                ...
                if result.get("success", True):
                    ...
                    if devices:
                        for device in devices:
                            status_lines.append(f"- {device.get('name')}: {device.get('status')}")
                    else:
